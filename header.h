...
int calculateRowPenalty(const vector<int> &topValues, PenaltyCalculationStrategy pcStrat) {
    vector<int> uniqueValues = topValues;
...
int calculateColPenalty(const vector<int> &topValues, PenaltyCalculationStrategy pcStrat) {
    vector<int> uniqueValues = topValues;
    sort(uniqueValues.begin(), uniqueValues.end());
    uniqueValues.erase(unique(uniqueValues.begin(), uniqueValues.end()), uniqueValues.end());
    ...
    for (int i = 0; i < costMatrix.size(); ++i) {
        ...
        for (int j = 0; j < costMatrix[i].size(); ++j) {
            if (costMatrix[i][j].included) {
                topValues.push_back(costMatrix[i][j].top);
            }
        }

        rowPenalties[i] = calculateRowPenalty(topValues, pcStrat);
    }

    for (int j = 0; j < costMatrix[0].size(); ++j) {
        vector<int> botValues;
        for (int i = 0; i < costMatrix.size(); ++i) {
            if (costMatrix[i][j].included) {
                botValues.push_back(costMatrix[i][j].bottom);
            }
        }

        colPenalties[j] = calculateColPenalty(botValues, pcStrat);
    }
}
...
void processFile(const fs::path &filePath, vector<Result> &results, PenaltyCalculationStrategy pcStrat, PenaltySelectionStrategy psStrat, PenaltyTieBreakerStrategy ptbStrat, CellSelectionStrategy csStrat, CellTieBreakerStrategy ctbStrat) {
    ...
    if (result < optimalSolution) cout << "Error detected!";
